...
class MHA(nn.Module):
    def __init__(self, channels, num_heads=8):
        ...
        self.conv_qkv = nn.Conv2d(in_channels=self.d, out_channels=3*self.d, kernel_size=(1, 1), stride=(1, 1))
    ...
    def forward(self, x):
        ...
        k = rearrange(q, 'b h d p t -> b h (d t) p')
        v = rearrange(q, 'b h d p t -> b h p (d t)')
...
class CTBlock(nn.Module):
    def __init__(self, channels, num_heads, E):
        ...
        self.bn1 = nn.BatchNorm3d(num_features=channels)
        self.cfe = CFE(channels=channels, E=E)
        self.bn2 = nn.BatchNorm3d(num_features=channels)
